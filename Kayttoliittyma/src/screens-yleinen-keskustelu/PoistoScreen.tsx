...
const PoistoScreen: FC<Props> = ({
    ...
    // const poistettavaAsia = viestiLista1.find(asia => asia.id === id); Vanha propseihin perustuva tapa
    ...
    const haePoistettavaAsia = async () => {
        try {
            const response = await fetch(`/foorumi1/${id}`);
   ...
   */
    /*const kasittelePoisto = (id: string) => {
        const uusiLista = viestiLista1.filter(asia => asia.id !== id);
    ...
    const kasittelePoisto = async (id: string) => {
        try {
            await fetch(`/foorumi1/${id}`, {
                method: "DELETE"
            })
            navigoi("/kategoriat/yleinen");
        } catch (error) {
    ...
    return (
        ...
        <>
         <Stack style={{border: "5px solid black", backgroundColor: "white", width: "960px", textAlign: "center", alignItems: "center"}}>
            ...
            <Button style={{scale: "1", width: "200px", marginLeft: "10px", marginBottom: "10px", border: "4px solid black"}} variant="contained" color="error" onClick={() => navigoi("/kategoriat/yleinen")}>Peruuta</Button>
        </Stack>
        </>
    )
}
