...
import LuontiScreen from "./LuontiScreen";
...
const ListaScreen: FC<Props> = ({
    ...
    const haeLista = async () => {
        try {
            const response = await fetch("/foorumi1");
    ...
    return (
        ...
        <Stack  fontFamily={"Roboto"} display={"inline-block"} border={"5px solid black"} width={"960px"} marginTop={"2px"} style={{backgroundColor: "white"}} textAlign={"center"}>
            <h1>Tervetuloa yleiselle keskustelu sivulle</h1>
            ...
            <Lista>
                { tehtavalista.map(asia => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <Button variant="contained" color="secondary" style={{scale: "1", width: "500px", marginLeft: "164px"}} onClick={ () => kasitteleMuokkaaNappi(asia.id) }>Muokkaa</Button>
                            
                            { asia.tila === "Älä tykkää" ?<ThumbDown
                                    style={{ color: "red", cursor: "pointer", scale: "3", marginTop: "122px", marginLeft: "40px", marginBottom: "10px" }}
                ...
                <LuontiScreen viestiLista1={[]} setViestiLista={function (lista: Viesti[]): void {} }/>
                </Stack>
