...
export default class TehtavalistaTietokantaModel4 {
    ...
    /**
     * Hakee Foorumin tietokannasta
     * 
     * @returns Foorumi
     */
    public haeTehtavalista = async () => {
        const tehtavat = await this.tietokanta?.all("SELECT * FROM foorumi4");
...
public async luoTehtava(otsikko: string, viesti: string) {
    ...
    if (!uniikkiOtsikko) {
        throw {
            statusKoodi: 409,
            virheViesti: `Foorumi sisältää jo otsikon nimellä ${otsikko}`
        }
    }
    ...
    const lause = await this.tietokanta?.prepare(`
        INSERT INTO foorumi4 (id, otsikko, viesti, tila)
        VALUES (?, ?, ?, ?)
    `)
    ...
    public async muokkaaTehtava(id: string, tehtava: Tehtava) {
        ...
        if(!uniikkiOtsikko) {
            if (loydettyTehtava.otsikko !== tehtava.otsikko){
                throw {
                    statusCode: 409,
                    virheViesti: `Foorumi sisältää jo tehtävän nimellä ${tehtava.otsikko}`
                }
            }
        }
    ...
    public etsiTehtava = async (id: string) => {
        const loydettyTehtava = await this.tietokanta?.get(`SELECT * FROM foorumi4 WHERE id = ?`, id) // "${id} kysymysmerkin tilalle"
    ...
    */
    private tarkistaOnkoOtsikkoKonflikti = async (otsikko: string) => {
        const loytyykoTehtava = await this.tietokanta?.get(`SELECT * FROM foorumi4 WHERE otsikko = ?`, otsikko)
    ...
    private yhdistaTietokantaan = async () => {
        ...
        await this.tietokanta.run(`
            CREATE TABLE IF NOT EXISTS foorumi4 (
