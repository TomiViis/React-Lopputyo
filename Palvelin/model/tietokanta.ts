...
export default class TehtavalistaTietokantaModel {
    ...
    public haeTehtavalista = async () => {
        const tehtavat = await this.tietokanta?.all("SELECT * FROM foorumi1");
...
public async luoTehtava(otsikko: string, viesti: string) {
    ...
    const lause = await this.tietokanta?.prepare(`
        INSERT INTO foorumi1 (id, otsikko, viesti, tila)
        VALUES (?, ?, ?, ?)
    `)
    ...
    /*
    await this.tietokanta?.exec(`
        DELETE FROM foorumi1 WHERE id = '${id}'
    `) */
    }
    ...
    public etsiTehtava = async (id: string) => {
        const loydettyTehtava = await this.tietokanta?.get(`SELECT * FROM foorumi1 WHERE id = ?`, id) // "${id} kysymysmerkin tilalle"
    ...
    */
    private tarkistaOnkoOtsikkoKonflikti = async (otsikko: string) => {
        const loytyykoTehtava = await this.tietokanta?.get(`SELECT * FROM foorumi1 WHERE otsikko = ?`, otsikko)
    ...
    private yhdistaTietokantaan = async () => {
        ...
        await this.tietokanta.run(`
            CREATE TABLE IF NOT EXISTS foorumi1 (
