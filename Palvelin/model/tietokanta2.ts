...
export default class TehtavalistaTietokantaModel2 {
    ...
    public haeTehtavalista = async () => {
        const tehtavat = await this.tietokanta?.all("SELECT * FROM foorumi2");
...
public async luoTehtava(otsikko: string, viesti: string) {
    ...
    const lause = await this.tietokanta?.prepare(`
        INSERT INTO foorumi2 (id, otsikko, viesti, tila)
        VALUES (?, ?, ?, ?)
    `)
    ...
    public etsiTehtava = async (id: string) => {
        const loydettyTehtava = await this.tietokanta?.get(`SELECT * FROM foorumi2 WHERE id = ?`, id) // "${id} kysymysmerkin tilalle"
    ...
    */
    private tarkistaOnkoOtsikkoKonflikti = async (otsikko: string) => {
        const loytyykoTehtava = await this.tietokanta?.get(`SELECT * FROM foorumi2 WHERE otsikko = ?`, otsikko)
    ...
    private yhdistaTietokantaan = async () => {
        ...
        await this.tietokanta.run(`
            CREATE TABLE IF NOT EXISTS foorumi2 (
